...
def build_vocab(path_dir, min_count_word=1, min_count_tag=1) :
    ...
    tags = Counter()
    size_train_tags = update_vocab(os.path.join(path_dir, 'train/labels.txt'), tags)
    #size_dev_tags = update_vocab(os.path.join(path_dir, 'dev/labels.txt'), tags)
    #size_test_tags = update_vocab(os.path.join(path_dir, 'test/labels.txt'), tags)
    print("- done.")
    ...
    tags_count = [str(count)  for tok, count in tags.items() if count >= min_count_tag]
